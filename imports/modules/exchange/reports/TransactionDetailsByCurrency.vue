    ...
    >
      ...
      <template #header>
        <div class="report-name">
          <!-- {{ reportName }} -->
          {{ $t('exchange.tranReport.transaction-details') }}
        </div>
      ...
      <div v-loading="loading">
        ...
        >
          ...
          >
            ...
            >
              <td>
                <span v-html="indentSpace(1)" />{{ formatDate(item.tranDate) }}
              </td>
              <td>
                <span
                  v-if="currentBranchId == item.branchId"
                  class="ra-text-link"
                  @click="refClick(item)"
                >
                  {{ item.refNo }}
              ...
              <td>{{ $t(`exchange.tableLabel.${item.type}`) }}</td>
              <!--<td class="text-right">
                {{ $filters.exchangeBaseCurrency(item.rate,item.rateFormat) }}
              </td>-->
              <td class="text-right">
                {{ item.rate.toLocaleString() }} {{ item.rateFormat }}
              </td>
              <td class="text-right">
                {{ item.received.toLocaleString() }} {{ item.fromSymbol }}
              </td>
              <!--<td class="text-right">
                {{
              ...
              <td class="text-right">
                {{ item.exchangeAmount.toLocaleString() }} {{ item.fromSymbol }}
              </td>
              ...
              <td class="text-right">
                {{ item.amount.toLocaleString() }} {{ item.toSymbol }}
              </td>
              <td class="text-right">
                {{ item.return.toLocaleString() }} {{ item.fromSymbol }}
              </td>
              <!--<td class="text-right">
                {{ tem.netIncome }}{{ item.fromSymbol }}
              </td>-->
              <td class="text-right">
                {{ item.netIncome.toLocaleString() }} {{ item.fromSymbol }}
              </td>
            </tr>
            <!-- TOTAL -->
            <tr>
              ...
              <td class="text-right text-bold ra-text-underline">
                {{ doc.totalReceived.toLocaleString() }} {{ doc.fromSymbol }}
              </td>
              <td class="text-right text-bold ra-text-underline">
                {{ doc.totalExchangeAmount.toLocaleString() }}
                {{ doc.fromSymbol }}
              </td>
              <td class="text-right text-bold ra-text-underline">
                {{ doc.totalAmount.toLocaleString() }} {{ doc.toSymbol }}
              </td>
              <td class="text-right text-bold ra-text-underline">
                {{ doc.totalReturn }} {{ doc.fromSymbol }}
              </td>
              <td class="text-right text-bold ra-text-underline">
                {{ doc.totalNetIncome.toLocaleString() }} {{ doc.fromSymbol }}
              </td>
            </tr>
          </tbody>
        </table>
...
import { reportTransactionDetailsByCurrency } from '../api/reports/exchange-transaction-details-byCurrency'
// Components
import {
  ElCard,
  ElForm,
    ...
    submitForm() {
      this.$refs['form'].validate((valid) => {
        if (valid) {
          ...
          reportTransactionDetailsByCurrency
